...
# Configuration now handled via Hydra - see conf/config_4m_fsdp.yaml
...
def setup_data(cfg: DictConfig):
    ...
    cfg.in_domains = sorted(set.union(*[set(cfg['in_domains'].split('-')) for cfg in train_config.values()]))
    cfg.out_domains = sorted(set.union(*[set(cfg['out_domains'].split('-')) for cfg in train_config.values()]))
    ...
    for dataset_name, dataset_cfg in train_config.items():
        ...
        dataset_batch_size = None # if num_trainsets > 0 else cfg.batch_size
        epoch_size = None # if num_trainsets > 0 else cfg.epoch_size
...
def get_model(cfg, modality_info):
...
@hydra.main(config_path="../conf", config_name="config_4m_fsdp", version_base=None)
def main(cfg: DictConfig):
    ## Distributed init
    # Convert DictConfig to OmegaConf for compatibility
    ...
    if cfg.cooldown_epochs < 0 and cfg.cooldown_steps < 0:
        if cfg.cooldown_tokens < 0 and cfg.lr_schedule in ['inverse_sqrt']:
    ...
    fsdp_utils.auto_load_model_fsdp(
        args=cfg, model=model, optimizer=optimizer)
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            skip_nan_grad=cfg.skip_nan_grad,
        )
        if cfg.output_dir:
            if (epoch + 1) % cfg.save_ckpt_freq == 0 or epoch + 1 == cfg.epochs:
                fsdp_utils.save_model_fsdp(args=cfg, model=model, optimizer=optimizer, epoch=epoch)
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    ...
                    frozen_model_epochs: int, accum_iter, max_norm: Optional[float] = None, log_writer=None,
                    lr_scheduler=None, start_steps: int = 0, lr_schedule_values=None, wd_schedule_values=None,
